...
from .cross_layer_v2 import CrossLayerV2 as CrossLayer
...
class CrossTest(tf.test.TestCase):
  ...
  def test_full_matrix(self):
    ...
    layer = CrossLayer(projection_dim=None, kernel_initializer="ones")
    output = layer(x0, x)
  ...
  def test_low_rank_matrix(self):
    x0 = np.asarray([[0.1, 0.2, 0.3]]).astype(np.float32)
    x = np.asarray([[0.4, 0.5, 0.6]]).astype(np.float32)
    layer = CrossLayer(projection_dim=1, kernel_initializer="ones")
    output = layer(x0, x)
    self.evaluate(tf.compat.v1.global_variables_initializer())
    self.assertAllClose(np.asarray([[0.55, 0.8, 1.05]]), output)
  ...
  def test_invalid_proj_dim(self):
    with self.assertRaisesRegexp(ValueError,
      ...
      layer = CrossLayer(projection_dim=6)
      layer(x0, x)
  ...
  def test_save_model(self):
    ...
    def get_model():
      ...
      x1 = CrossLayer(projection_dim=None)(x0, x0)
      x2 = CrossLayer(projection_dim=None)(x0, x1)
